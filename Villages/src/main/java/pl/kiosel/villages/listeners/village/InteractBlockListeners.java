...
public class InteractBlockListeners implements Listener {
	...
	public InteractBlockListeners(Wioski plugin) {
		this.plugin = plugin;
	...
	private boolean isInEnabledWorld(Location loc) {
		...
		return !plugin.getApi().getEnabledWorlds().contains(loc.getWorld().getName());
	}
	...
	public void onInteractTurret(PlayerInteractEvent event) {
		...
		if (village != null) {
			...
			event.setCancelled(true);
			if (type == Material.NOTE_BLOCK) return;
			player.playSound(player.getLocation(), Sound.BLOCK_ANVIL_LAND, 0.1f, 1.0f);
		}
	}
