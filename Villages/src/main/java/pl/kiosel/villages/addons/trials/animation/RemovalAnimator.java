...
public final class RemovalAnimator {
    ...
    public long play(Village village) {
        ...
        ZParticle.SMOKE.spawn(center, 24, 0.3, 0.3, 0.3, 0.01);
        ...
        new BukkitRunnable() {
            ...
            public void run() {
                ...
                if (tick % settings.getBeepIntervalTicks() == 0) {
                    ...
                    settings.getBeepSound().play(center, settings.getBeepVolume(), pitch);
                    ZParticle.CRIT.spawn(center.clone().add(0, 0.8, 0),
                            8, 0.2, 0.2, 0.2, 0.01);
                }

                if (tick >= duration) {
                    ZParticle.EXPLOSION.spawn(center, 3, 0.25, 0.25, 0.25, 0);
                    pushPlayers(center);
                    cancel();
                    return;
                }
