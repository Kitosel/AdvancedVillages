...
public final class DiplomacyManager {
	...
	private final Set<UUID> removedRequestIds = ConcurrentHashMap.newKeySet();
	...
	public void load() {
		...
		for (AllianceRequest request : loaded.getRequests()) {
			if (!this.validPair(request.getSenderVillageId(), request.getTargetVillageId())) {
				this.removedRequestIds.add(request.getId());
				invalidData = true;
				continue;
			}
			AllianceRequest previous = this.requests.put(
					requestKey(request.getSenderVillageId(), request.getTargetVillageId()), request);
			if (previous != null) {
				this.removedRequestIds.add(previous.getId());
				invalidData = true;
			}
		}
	...
	public void save(boolean ignoreUnchanged) {
		...
		Set<UUID> removedRequests = new HashSet<>(this.removedRequestIds);
		Set<UUID> removedWars = new HashSet<>(this.removedWarIds);
		try {
			this.storage.save(new ArrayList<>(this.alliances.values()),
					new ArrayList<>(this.requests.values()), warSnapshots,
					removedAlliances, removedRequests, removedWars);
			this.removedAllianceIds.removeAll(removedAlliances);
			this.removedRequestIds.removeAll(removedRequests);
	...
	public DiplomacyResult requestAlliance(Village sender, Village target) {
		...
		if (!current.isEnabled() || !current.isAlliancesEnabled()) return DiplomacyResult.DISABLED;
		...
		Instant now = Instant.now();
		AllianceRequest request = new AllianceRequest(null, sender.getUUID(), target.getUUID(),
	...
	public DiplomacyResult acceptAlliance(Village target, Village sender) {
		...
		if (request == null || request.isExpired(Instant.now())) {
			if (request != null) {
				if (this.requests.remove(requestKey, request)) this.removedRequestIds.add(request.getId());
				this.markChanged();
			}
	...
	public DiplomacyResult denyAlliance(Village target, Village sender) {
		if (!this.settings.isEnabled() || !this.settings.isAlliancesEnabled()) return DiplomacyResult.DISABLED;
		String key = requestKey(sender.getUUID(), target.getUUID());
		AllianceRequest removed = this.requests.remove(key);
		if (removed == null) return DiplomacyResult.NO_REQUEST;
		this.removedRequestIds.add(removed.getId());
		this.markChanged();
	...
	public DiplomacyResult declareWar(Village attacker, Village defender) {
		this.refreshTransitions(true);
		DiplomacySettings current = this.settings;
		if (!current.isEnabled() || !current.isWarsEnabled()) return DiplomacyResult.DISABLED;
	...
	public DiplomacyResult surrender(Village village, Village enemy) {
		if (!this.settings.isEnabled() || !this.settings.isWarsEnabled()) return DiplomacyResult.DISABLED;
	...
	public DiplomacyAttackResult canAttack(@Nullable Village attacker, Village defender) {
		DiplomacySettings current = this.settings;
		if (!current.isEnabled()) return DiplomacyAttackResult.ALLOWED;
	...
	public void removeVillage(Village village) {
		...
		for (Map.Entry<String, AllianceRequest> entry : new ArrayList<>(this.requests.entrySet())) {
			AllianceRequest request = entry.getValue();
			if ((request.getSenderVillageId().equals(villageId) || request.getTargetVillageId().equals(villageId))
					&& this.requests.remove(entry.getKey(), request)) {
				this.removedRequestIds.add(request.getId());
			}
		}
		this.markChanged();
	...
	private void refreshTransitions(boolean broadcast) {
		...
		for (Map.Entry<String, AllianceRequest> entry : new ArrayList<>(this.requests.entrySet())) {
			AllianceRequest request = entry.getValue();
			if (request.isExpired(now) && this.requests.remove(entry.getKey(), request)) {
				this.removedRequestIds.add(request.getId());
				changed = true;
			}
		}
	...
	private void removeRequestsBetween(UUID first, UUID second) {
		AllianceRequest direct = this.requests.remove(requestKey(first, second));
		AllianceRequest reverse = this.requests.remove(requestKey(second, first));
		if (direct != null) this.removedRequestIds.add(direct.getId());
		if (reverse != null) this.removedRequestIds.add(reverse.getId());
	}
