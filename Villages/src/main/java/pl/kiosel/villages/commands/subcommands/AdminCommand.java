...
public class AdminCommand extends AVSubCommand {
	...
	public void run(Player player, User user, String[] args) {
		...
		if (input.equals(manageCmd)) {
			...
			String owner = args[2];
			User userOwner = plugin.getUserManager().findByName(owner).get();
			...
			if (args.length == 6) {
				...
				if (manage.equals(bankCmd)) {
					if (addrem.equals(addCmd)) {
						...
						VillageUtilsManager.replaceWith(player, village, Lang.COMMAND_ADMIN_BANK_ADD.getPath())
								.processPlaceholder("money", number)
								.sendPrefixedMessage(player);
					}
					if (addrem.equals(removeCmd)) {
						village.removeBank(number);
						VillageUtilsManager.replaceWith(player, village, Lang.COMMAND_ADMIN_BANK_REMOVE.getPath())
								.processPlaceholder("money", number)
								.sendPrefixedMessage(player);
					}
					return;
				...
				if (manage.equals(livesCmd)) {
					if (addrem.equals(addCmd)) {
						...
						VillageUtilsManager.replaceWith(player, village, Lang.COMMAND_ADMIN_LIVES_ADD.getPath())
								.processPlaceholder("lives", number)
								.sendPrefixedMessage(player);
					}
					if (addrem.equals(removeCmd)) {
						if (village.getLives() < 1) {
							player.sendMessage("Village is already dying");
							return;
						}
						village.setLives(village.getLives() - number);
						VillageUtilsManager.replaceWith(player, village, Lang.COMMAND_ADMIN_LIVES_REMOVE.getPath())
								.processPlaceholder("lives", number)
								.sendPrefixedMessage(player);
					}
					return;
			...
			if (args.length == 7) {
				...
				if (manage.equals(protectionCmd)) {
					if (addrem.equals(addCmd)) {
						...
						VillageUtilsManager.replaceWith(player, village, Lang.COMMAND_ADMIN_PROTECTION_ADD.getPath())
		...
		player.sendMessage(tl("&cUnknown command: &f" + input));
	}
}