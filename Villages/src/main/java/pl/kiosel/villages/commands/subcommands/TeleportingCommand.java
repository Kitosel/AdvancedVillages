...
public class TeleportingCommand extends SubCommand {
	...
	@Override
	public String getUsage() { return "/village teleporting"; }

	@Override
	public String getPermission() { return "villages.command.teleporting"; }
			...
			if (args[1].equalsIgnoreCase("village7"))
				if (args[2].equalsIgnoreCase("set9")) {
					if (plugin.getTeleportManager().isTeleportTask(player)) {
						...
						player.sendTitle(
								plugin.getLang().getMessage(Lang.TELEPORT_SET_TITLE),
								plugin.getLang().getMessage(Lang.TELEPORT_SET_SUBTITLE),
								10, 30, 10);
