...
//		plugin.getDataHelper().createVillage(village);
	...
	public Village getVillageAt(Location loc) {
		...
		for (Village village : manager.getVillagesView()) {
			...
			if (villageLocation == null || villageLocation.getWorld() == null
					|| !villageLocation.getWorld().equals(loc.getWorld())) continue;
	...
	public boolean isSpawnNearby(Location location, int radius) {
		Location vLoc = plugin.getSpawnManager().getSpawn().orElse(null);
	...
	public static String replaceWith(Village village, String string) {
		...
		return ColorUtils.tl(string.replace("%village_level%", village == null ? "" : village.getLevel().getLevel()+"")
				...
				.replace("%village_animations%", village == null ? "" : village.isAnimationsEnabled() ? GuiConfig.on : GuiConfig.off)
	...
	public static Message replaceWith(Player player, Village village, String message) {
		...
		return AdvancedVillages.getInstance().getLocale().newMessage(message)
				...
				.processPlaceholder("village_animations", village == null ? noVillage : village.isAnimationsEnabled() ? GuiConfig.on : GuiConfig.off)
