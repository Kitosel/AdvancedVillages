...
public class TeleportManager {
	...
	public boolean teleportPlayerToVillage(Player player) {
		...
		if (village == null || village.getLocation() == null) {
		...
		BukkitRunnable teleportTask = new BukkitRunnable() {
			...
			public void run() {
				...
				Village v = plugin.getUserManager().findByPlayer(player).get().getPresentVillage();
				if (v == null || v.getHome().get() == null) {
					locale.getMessage(Lang.VILLAGE_NO.getPath()).sendPrefixedMessage(player);
					return;
				}
				player.teleport(v.getHome().get());
				locale.getMessage(Lang.TELEPORTED.getPath()).processPlaceholder("village", v.getName()).sendPrefixedMessage(player);
			}
		};
