...
public class UpgradeInventory extends VillageMenu {
	...
	public Inventory getInventory(Village village, Player player) {
		...
		ItemStack upgrade = create(getMaterialUpgrade(village.getLevel().getLevel()), GuiConfig.guis_village_upgrade_button, upgrade_list);
		...
		switch (village.getLevel().getLevel()) {
			case 1:
		...
		if (village.getLevel().getLevel() != 5) {
