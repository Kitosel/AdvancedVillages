...
public final class MemberSettingsGUI extends VillageMenu {
	...
	public MemberSettingsGUI(AdvancedVillages plugin, VillageGUIManager menus, Village village,
		...
		if (remove.isEnabled()) {
			setButton(remove.getSlot(), remove.createItem(), event -> event.getManager().showGUI(event.getPlayer(),
					new MemberRemoveGUI(plugin, village, viewer, this, memberId)));
		}
	}
	...
	private void changeRole(User member, int direction) {
		...
		plugin.getGuiManager().showGUI(viewer,
				new MemberSettingsGUI(plugin, menus, village, viewer, getParent(), memberId));
	}
}
