...
public final class ResidentGUI extends VillageMenu {
	...
	public ResidentGUI(AdvancedVillages plugin, VillageGUIManager menus, Village village,
		...
		if (plugin.isDev())
			setButton(3, 5, ItemCreator.of(ZMaterial.RED_DYE).makeMenuItem(),
					event -> plugin.getGuiManager().showGUI(event.getPlayer(), new PermissionGUI(plugin, menus, village, player, this)));
	}
