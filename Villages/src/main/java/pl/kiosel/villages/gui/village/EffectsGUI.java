...
public final class EffectsGUI extends VillageMenu {

	public EffectsGUI(AdvancedVillages plugin, VillageGUIManager menus, Village village,
	...
	private void toggle(BooleanSupplier purchased, BooleanSupplier active, Runnable action,
		...
		if (!hasPermission(VillagePermission.EFFECTS_TOGGLE)) return;
	...
	private void buy(BooleanSupplier purchased, Runnable action, double price, String setting) {
		if (!hasPermission(VillagePermission.EFFECTS_BUY)) return;
		if (purchased.getAsBoolean()) {
