...
import pl.kiosel.villages.data.village.features.quest.QuestType;
...
public final class BankGUI extends VillageMenu {

	public BankGUI(AdvancedVillages plugin, VillageGUIManager menus, Village village,
	...
	private void addBank(int amount) {
		if (!hasPermission(VillagePermission.BANK_ADD)) {
			return;
		}
	...
	private void removeBank(int amount) {
		if (!hasPermission(VillagePermission.BANK_REMOVE)) {
			return;
		}
